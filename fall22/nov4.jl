...
function createMonomial4Order(a, b)
    @polyvar x y 
    X2 = monomials([x, y], 0:4)
    p = subs(X2, x => a, y=> b)
    return p
end
...
function createA(xVect, yVect)
    A = Array{Float64}(undef, length(xVect), 15)
    index = 1
    for q in length(xVect)
        h = createMonomial4Order(xVect[q], yVect[q])
        for r in length(h)
...
function createAGen()
    A = Array{Float64}(undef, 5, 15)
    ind = 1
    for q in (1:5)
        h = createMonomial4Order(q, q)
        for r in length(h)
            temp = convert(Float64, h[r])
            A[ind] = temp
            ind += 1
...
function getMomentsArray(data)
    A = Array{Float64}(undef, 5, 15)
    for q in (1:5)
        h = getMomentsVector(data, q)
        for r in length(h)
            temp = convert(Float64, h[r])
            A[index] = temp
            index += 1
...
function getMomentsVector(data, index)
    data1 = data[1]
    data2 = data[2]
    data3 = data[3]
    data4 = data[4]
    return [data4[index,1,1,1,1], data4[index,1,1,1,2], data4[index,1,1,2,2], 
        data4[index,1,2,2,2], data4[index,2,2,2,2], 
        data3[index,1,1,1], data3[index,1,1,2], data3[index,1,2,2], data3[index,2,2,2], 
        data2[index,1,1], data2[index,1,2], data2[index,2,2],
        data1[index,1], data1[index, 2], 0]
end
...
function main(std) 
    ...
    z2 = calcZ2(x,y)
    #get moments of [x, y]
    A = getMomentsArray(z2)
    #regression of [x, y] with sin(x)*cos(y)
    b = leastSquaresReg(A, z)
end
